...
class PaymentController {
    constructor() {
        this.paymentService = new payment_service_js_1.PaymentService();
        /**
        ...
        this.processPayment = async (req, res) => {
            try {
                ...
                console.error(`[CONTROLLER] Error in processPayment for user ${req.user?.userId}:`, error);
                res.status(500).json({
                    success: false,
        ...
        this.getPaymentHistory = async (req, res) => {
            try {
                ...
                const transactions = await this.paymentService.getPaymentHistory(userId, limit ? parseInt(limit) : 10, offset ? parseInt(offset) : 0);
                res.json({
                    success: true,
                    data: transactions,
         ...
         */
        this.validateAmount = async (req, res) => {
            try {
